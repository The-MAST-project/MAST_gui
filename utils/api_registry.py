...
CONFIG_ENDPOINTS = [
    ...
    GuiEndpoint(
        "control", "/config/set_unit/{current_site}/{unit_name}", "POST", "canChangeConfiguration", "Set unit config"
    ),
]
...
UNIT_ENDPOINTS = [
    ...
    GuiEndpoint(
        "control",
        "/unit/{unit_name}/power_switch/set_outlet/{outlet_id}/{state}",
        "PUT",
        "canUseControls",
        "Set outlet state",
    ),
...
GUI_ENDPOINTS = CONTROL_ENDPOINTS + CONFIG_ENDPOINTS + UNIT_ENDPOINTS + TASK_ENDPOINTS


def is_endpoint_allowed(service: str, path: str, method: str) -> bool:
    """Check if an endpoint is allowed to be called from GUI"""
    return any(ep.service == service and ep.path == path and ep.method == method for ep in GUI_ENDPOINTS)
