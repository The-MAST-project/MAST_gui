    ...
    def wrapper(request, *args, **kwargs):
        ...
        if isinstance(result, tuple) and len(result) >= 2:
            ...
            if backend_method is None:
                return JsonResponse({"error": f"Backend method {method_name} not found"}, status=500)

            # Check if method is GUI-exposed
            if not is_gui_endpoint(backend_method):
                return JsonResponse({"error": f"Endpoint {method_name} not exposed to GUI"}, status=403)
            ...
            if required_cap and not user_has_capability(request.user, required_cap):
                return JsonResponse({"error": f"Missing required capability: {required_cap}"}, status=403)
            ...
            if hasattr(response, "succeeded"):
                if response.succeeded:
                    ...
                    return JsonResponse({"success": False, "errors": response.errors}, status=400)
