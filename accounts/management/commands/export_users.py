...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        users = [{"uuid": str(u.uid), "display": u.display} for u in User.objects.filter(is_active=True).exclude(display="")]
        USERS_JSON.write_text(json.dumps(users, indent=2))
        self.stdout.write(self.style.SUCCESS(f"Exported {len(users)} users → {USERS_JSON}"))
