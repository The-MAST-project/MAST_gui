...
def _build_capabilities(user):
    return [
        {"code": code, "name": name, "has_permission": user.has_perm(f"accounts.{code}")} for code, name in _CAPABILITIES
    ]
        ...
        if username:
            try:
                ...
                if not user.email_verified:
                    ...
                    form.add_error(
                        None, "Your account is awaiting admin approval. You will be notified by email when it is approved."
                    )
                ...
                form.errors.clear()
                form.add_error(None, f'No account found for "{username}". You may sign up first.')
        return super().form_invalid(form)
...
def _send_verification_email(request, user):
    verify_url = request.build_absolute_uri(reverse("accounts:verify_email", args=[user.email_verification_token]))
    send_mail(
...
def admin_approve_edit(request, user_id):
    user = get_object_or_404(User, id=user_id)
    return _user_edit(
        request, user, post_url=reverse("admin_approve_edit", args=[user_id]), submit_label="Activate", approve=True
    )
...
def _user_edit(request, user, post_url, submit_label="Save", approve=False):
    ...
    caps = sorted(set(perm.split(".", 1)[1] for perm in user.get_group_permissions() if perm.startswith("accounts.")))
    return render(
        request,
        "admin/partials/user_edit_modal.html",
        {
            "edit_user": user,
            "form": form,
