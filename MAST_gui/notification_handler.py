...
def broadcast_activity_indicators_update():
    ...
    for site_name, site_status in sites_status.sites.items():
        ...
        activities_update = {"type": "activity_indicators_refresh", "site": site_name, "components": {}}

        # Add controller activities
        if hasattr(site_status, "controller") and site_status.controller:
            activities_update["components"]["controller"] = getattr(site_status.controller, "activities_verbal", [])

        # Add spec activities
        if hasattr(site_status, "spec") and site_status.spec:
            activities_update["components"]["spec"] = getattr(site_status.spec, "activities_verbal", [])

        # Add unit activities
        if hasattr(site_status, "units") and site_status.units:
            for unit_name, unit_status in site_status.units.items():
                activities_update["components"][unit_name] = getattr(unit_status, "activities_verbal", [])
